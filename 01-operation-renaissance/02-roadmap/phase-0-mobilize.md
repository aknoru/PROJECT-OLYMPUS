---
id: REN-ROAD-PHASE-0
title: "Phase 0: Mobilize"
type: phase
status: approved
version: 0.1.0
owner: project-maintainer
---

# Phase 0: Mobilize

## Purpose

Commission the campaign environment and authorize launch.

## Scope

This phase governs campaign Days 1–7. Calendar dates are generated from configuration.

## Theory

A lifecycle phase isolates a coherent objective and ends with evidence-based review. Calendar passage does not satisfy a gate.

## Framework

| Element | Rule |
|---|---|
| Relative days | Days 1–7 |
| Expected outcomes | Verified environment, baseline, charter, capacity estimate, and risk inventory |
| Deliverables | preflight evidence; baseline record; signed commitment status; first weekly plan |
| KPIs | Evidence completeness, gate status, capacity adherence, open risk count |
| Entry criteria | Master Architecture and Sprint 03A foundation approved |
| Exit criteria | launch blockers resolved; configuration and capacity verified |
| Risks | setup expansion, missing backup, inaccurate availability |
| Dependencies | Foundation and Quickstart |

## Workflow

Confirm entry evidence; allocate capacity; execute only phase deliverables; review risks weekly; assemble the exit package; conduct the gate; record advance, remediate, or stop.

## Decision Tree

```mermaid
flowchart TD
    A["Trigger"] --> B{"Required evidence available?"}
    B -- "No" --> C["Collect minimum evidence"]
    B -- "Yes" --> D{"Within constraints?"}
    C --> D
    D -- "No" --> E["Reduce, defer, or escalate"]
    D -- "Yes" --> F["Execute and verify"]
    E --> G["Record decision"]
    F --> G
```

Advance only when mandatory exit evidence is verified. A date boundary triggers review, not automatic promotion.

## Failure Modes

tool shopping, ceremonial planning, and premature study load

## Recovery

use manual tools, repair blocking checks, repeat the dry run

## Examples

Example: preflight evidence is accepted only when its evidence link and reviewer are recorded.

## Tables

The framework table is normative. Local values belong in configuration or cycle
records, not this protocol.

## Mermaid Diagrams

The decision tree above defines the control flow for this protocol.

## Cross References

- [184-Day Roadmap](184-day-roadmap.md)
- [Milestones](milestones.md)
- [Capacity Model](capacity-model.md)

## References

- `SRC-NASA-SE-2016`
- `SRC-LITTLE-1961`
- `SRC-RUBINSTEIN-2001`
- [Source register](../../references/source-register.yml)

## Next Steps

Proceed to Phase 1: Foundations only after the gate decision.

## Acceptance Criteria

- [x] Inputs, decisions, evidence, and recovery are explicit.
- [x] No external date or outcome is fabricated.
