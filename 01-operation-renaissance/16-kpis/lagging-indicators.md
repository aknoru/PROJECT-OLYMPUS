...
Define delayed evidence and external outcomes with uncertainty and limited causal interpretation.

## Scope

This document covers assessment results, released artifacts, campaign outcomes, external responses, and delayed system effects. Metric definitions live in the canonical registry; observations remain in approved source records. It does not hardcode personal goals,
...
Identify the outcome and delay, verify provenance, record coverage and confounders, compare only with configured context, and use the result to update strategy rather than assign blame.
...
An interview invitation is recorded as an external response, not proof that one application action caused it.
