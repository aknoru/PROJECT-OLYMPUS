...
Show the daily execution state without becoming a task-entry source.

## Scope

This document covers date, available capacity, primary deliverable, secondary work, evidence links, blockers, guardrails, and shutdown state. Dashboards are derived views and never own source data. It does not hardcode personal goals,
...
Read the daily plan and current evidence, check freshness, show no more than five primary measures, surface blockers, and route the daily-close decision.
...
A planned block appears as pending until its canonical evidence link resolves; the dashboard does not infer completion.
