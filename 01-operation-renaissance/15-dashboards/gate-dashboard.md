...
Provide configurable examination-preparation visibility without hardcoded syllabus, dates, marks, or targets.

## Scope

This document covers versioned syllabus coverage, diagnostic evidence, practice, revision, mock review, error recurrence, and source freshness. Dashboards are derived views and never own source data. It does not hardcode personal goals,
...
Read current configured examination inputs and canonical practice evidence, validate the cycle version, aggregate by configured subject keys, and route remediation.
...
A topic is marked insufficient-data when the current cycle lacks reviewed diagnostic evidence.
