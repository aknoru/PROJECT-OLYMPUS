...
Show role-to-evidence readiness and pipeline state without predicting selection.

## Scope

This document covers role families, competency evidence, resume review, application stages, interview evidence, decisions, and source dates. Dashboards are derived views and never own source data. It does not hardcode personal goals,
...
Read target roles and canonical records, verify current stage definitions, summarize controllable actions separately from external outcomes, and route the next pipeline decision.
...
Application activity and interview outcomes appear in separate layers so volume is not presented as selection probability.
...
- [Placement System](../06-placement-system/README.md)
